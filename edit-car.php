...
$id = $user_id = $car_brand = $license_plate = $model = $charging_type = $year = $battery_capacity = $is_charging = 0;
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $car_sql = "SELECT * FROM car WHERE id = ".$id;
    $car_result = $conn->query($car_sql);
    $car_record = $car_result->fetch_assoc();

    if(count($car_record) > 0){
        extract($car_record);
    } else {
